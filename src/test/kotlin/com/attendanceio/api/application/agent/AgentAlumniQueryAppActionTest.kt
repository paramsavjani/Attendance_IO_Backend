...
class AgentAlumniQueryAppActionTest {
    ...
    fun `a region expands to its cities and results are merged best-paying first`() {
        val repo = FakeRepo()
        val result = AgentAlumniQueryAppAction(repo).search(null, null, null, null, "Gujarat", false, 15)
    ...
    fun `company is resolved by name and an unknown one is reported not guessed`() {
        ...
        assertEquals(listOf("B", "C"), action.search(null, "goog", null, null, null, false, 15).items.map { it.name })
        val unknown = action.search(null, "Nowhere Inc", null, null, null, false, 15)
